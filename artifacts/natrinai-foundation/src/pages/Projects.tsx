      ...
      <section className="py-20 bg-gradient-to-br from-[#0D5D42] to-[#123D6A] relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <h1 className="text-4xl sm:text-5xl font-black text-white mt-3 mb-5 font-['Raleway']">All Projects</h1>
          ...
          <div className="flex flex-wrap gap-2">
            {categories.map((cat) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-semibold font-['Barlow'] transition-all ${
                  activeCategory === cat
                  ...
                  <div className="h-full bg-white rounded-3xl border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300">
                    ...
                    <div className="p-7">
                      <div className="flex items-center justify-between mb-4">
                        <span className="px-3 py-1 rounded-full text-xs font-semibold font-['Barlow']" style={{ backgroundColor: proj.color + "15", color: proj.color }}>
                          {proj.category}
                        </span>
                        <span className="px-3 py-1 rounded-full text-xs font-semibold font-['Barlow'] bg-gray-100" style={{ color: statusColors[proj.status] }}>
                          {proj.status}
                        </span>
                      </div>
                      <h3 className="font-black text-gray-900 text-lg mb-2 font-['Raleway'] group-hover:text-[#0D5D42] transition-colors leading-snug">{proj.title}</h3>
                      <p className="text-xs text-gray-400 mb-3 font-['Barlow']">{proj.location}</p>
                      <p className="text-gray-600 text-sm leading-relaxed font-['Barlow'] mb-5">{proj.desc}</p>
                      <div className="flex flex-wrap gap-2 mb-5">
                        {proj.tags.map((tag) => (
                          <span key={tag} className="px-2 py-1 bg-gray-50 rounded-lg text-xs text-gray-500 font-['Barlow']">{tag}</span>
                        ))}
                      </div>
