...
export default function Contact() {
  ...
  const inputCls = "w-full px-4 py-3 border border-gray-200 rounded-xl text-gray-800 text-sm font-['Inter'] outline-none focus:border-[#0D5D42] focus:ring-1 focus:ring-[#0D5D42]/20 transition-all";
  const labelCls = "block text-sm font-semibold text-gray-700 font-['Inter'] mb-1.5";
      ...
      <section className="py-20 bg-gradient-to-br from-[#0D5D42] to-[#123D6A] relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <span className="text-[#D79A1E] font-semibold text-sm tracking-widest uppercase font-['Montserrat']">Reach Out</span>
            <h1 className="text-4xl sm:text-5xl font-black text-white mt-3 mb-5 font-['Poppins']">Contact Us</h1>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-8 grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="space-y-5">
              {[
                ...
                <div key={item.label} className="flex gap-4 p-6 bg-white rounded-2xl border border-gray-100 hover:shadow-md transition-all">
                  ...
                  <div>
                    <p className="font-bold text-gray-900 text-sm font-['Poppins'] mb-1">{item.label}</p>
                    <p className="text-gray-600 text-sm font-['Inter']">{item.value}</p>
                  </div>
                </div>
              ))}

              <div className="p-6 bg-[#0D5D42] rounded-2xl text-white">
                <p className="font-bold font-['Poppins'] mb-2">CSR Partnerships</p>
                <p className="text-white/75 text-sm font-['Inter'] mb-3">Partner with us for your CSR program. We offer custom project alignment, regular reporting, and site visits.</p>
                <p className="text-[#D79A1E] text-sm font-semibold font-['Montserrat']">csr@natrinaifoundation.org</p>
              </div>

              <div className="p-6 bg-white rounded-2xl border border-gray-100">
                <p className="font-bold text-gray-900 font-['Poppins'] mb-3 text-sm">Follow Us</p>
                <div className="flex gap-3">
                  {["Facebook", "Twitter", "Instagram", "YouTube", "LinkedIn"].map((s) => (
                    <a key={s} href="#" className="px-3 py-2 bg-gray-100 hover:bg-[#0D5D42] hover:text-white text-gray-600 rounded-lg text-xs font-semibold font-['Montserrat'] transition-all">
                      {s.substring(0, 2)}
                    </a>
                  ))}
                </div>
          ...
          <div className="lg:col-span-2 space-y-7">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm">
              <h2 className="text-xl font-black text-[#0D5D42] font-['Poppins'] mb-6">Send Us a Message</h2>
              {contactSent ? (
                <div className="flex flex-col items-center py-10 text-center">
                  <CheckCircle className="w-14 h-14 text-[#5AAE3D] mb-4" />
                  <p className="text-xl font-black text-[#0D5D42] font-['Poppins'] mb-2">Message Received!</p>
                  <p className="text-gray-500 font-['Inter'] text-sm">We will get back to you within 24–48 hours.</p>
                  <button onClick={() => setContactSent(false)} className="mt-5 text-[#0D5D42] text-sm font-semibold font-['Inter'] underline">Send another message</button>
                </div>
              ) : (
                <form onSubmit={handleContact} className="space-y-5">
                  ...
                  <button type="submit" className="flex items-center gap-2 px-7 py-3.5 bg-[#0D5D42] text-white rounded-xl font-bold font-['Poppins'] hover:bg-[#0a4a35] transition-all" data-testid="button-contact-submit">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm">
              <h2 className="text-xl font-black text-[#0D5D42] font-['Poppins'] mb-2">Become a Volunteer</h2>
              <p className="text-gray-500 text-sm font-['Inter'] mb-6">Join our volunteer network and contribute your time and skills to our programs.</p>
              {volunteerSent ? (
                <div className="flex flex-col items-center py-10 text-center">
                  <CheckCircle className="w-14 h-14 text-[#5AAE3D] mb-4" />
                  <p className="text-xl font-black text-[#0D5D42] font-['Poppins'] mb-2">Thank You for Volunteering!</p>
                  <p className="text-gray-500 font-['Inter'] text-sm">Our team will reach out to you within 3–5 working days.</p>
                  <button onClick={() => setVolunteerSent(false)} className="mt-5 text-[#0D5D42] text-sm font-semibold font-['Inter'] underline">Submit another application</button>
                </div>
              ) : (
                <form onSubmit={handleVolunteer} className="space-y-5">
                  ...
                  <button type="submit" className="flex items-center gap-2 px-7 py-3.5 bg-[#5AAE3D] text-white rounded-xl font-bold font-['Poppins'] hover:bg-[#4a9633] transition-all" data-testid="button-vol-submit">
